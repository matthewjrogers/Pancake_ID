...
all_recipes %<>% # get rid of  "Dry Ingredients" and "Wet Ingredients" and similar
  filter(units != "wet",
         units != "dry",
         amount != is.na(amount)) %>% 
...
all_recipes %<>% # standardize variable units to cups
  ...
  mutate(std_units = case_when(str_detect_any(units, convertable) ~ "cups",
...
final_recipes <- all_recipes %>% 
  ...
  summarise(amount_sum = sum(std_amount), # collapse multiple observations into a single value (e.g. white sugar and brown sugar)
...
# Tidy Data (Recipe as Unique ID) -----------------------------------------

final_recipes %<>% 
  mutate(ingredient = str_replace(ingredient, " ", "_"), # tidy vup variable names preemptively
  ...
  spread(ingredient, amount_sum) %>% # turn each ingredient into its own variable
...
tidy_recipes <- all_recipes %>% 
  ...
  left_join(final_recipes) %>% # 
  distinct(recipe_id, .keep_all = T) %>% # the summarise_all call above created large observations for servings
